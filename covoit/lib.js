...
var Planner = {
	...
	init: function() {
		...
		if(!this.EVENTS) {
			...
			if(!localStorage.setup) {
				...
				// prepopulate the calendar with some holidays for the current month
	...
	initUI: function() {
		...
		$('#btn-ical').click(function() {
			Planner.exportData('simpleplanner.ics', 'text/calendar', Planner.exportIcal() );
			return false;
		});
		
		// ical
		$('#btn-csv').click(function() {
			Planner.exportData('simpleplanner.csv', 'text/csv', Planner.exportCSV() );
			return false;
		});
	...
	renderEvents: function() {
		...
		$('#stats').html( stats.future + ' prochain evenement et ' + stats.past + ' evenement passe' );
	},
	showDiary: function() {
		this.diary_open = true;
	...
	exportData: function(filename, mime, data) {
		var link = $('#popup-export .download');
		link.attr('download', filename);
		
		$('#popup-export .filename').html(filename);
		...
		link.attr('href',  window.URL.createObjectURL( builder.getBlob(mime) ) );
		
		if(navigator.userAgent.match(/Firefox/i)) {
			$('#popup-export .mozilla').show();
		} else {
			$('#popup-export .mozilla').hide();
		}

		Planner.dialog( $('#popup-export') );
		return false;
	},
	exportIcal: function() {
		var ical = '';
	...
	exportCSV: function() {
		...
		for(var id in this.EVENTS) {
			...
			$.each(this.EVENTS[id], function() {
				csv+= '"' + this.description + '",';
				csv+= '"' + date.getFullYear() + '-' + date.getMonth().pad(2) + '-' + date.getDate().pad(2) + '",';
				csv+= '"' + this.hour + ':' + this.minute + ':00' + '",';
				csv+= '"",'; // end date
				csv+= '"",'; // end time
				csv+= '"",'; // all day
				csv+= '"",'; // description
